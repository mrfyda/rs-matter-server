...
pub fn grant_ota_access(
    ...
    matter.with_state(|state| {
        ...
        if fabric
            .acl_iter()
            .any(|entry| grants_ota_access(entry, node_id))
        {
            return Ok(false);
        }
...
impl OtaImagesRegistry for ImageStore {
    async fn query<'b>(
        ...
        if designator_buf.len() < name.len() + UPDATE_TOKEN_LEN {
            log::warn!(
                "No room to name the image offered to {}/{}",
                vendor_id,
                product_id
            );
            return OtaQueryOutcome::NotAvailable;
...
mod tests {
    ...
    fn a_grant_is_added_once_and_recognised_afterwards() {
        ...
        let before =
            matter.with_state(|state| state.fabrics.get(fabric_index).unwrap().acl_iter().count());
        ...
        matter.with_state(|state| {
            ...
            assert_eq!(
                fabric
                    .acl_iter()
                    .filter(|e| grants_ota_access(e, 42))
                    .count(),
                1
            );
            assert_eq!(
                fabric
                    .acl_iter()
                    .filter(|e| grants_ota_access(e, 43))
                    .count(),
                1
            );
        });
    ...
    fn a_grant_is_recognised_by_node_and_cluster() {
        ...
        entry
            .add_target(Target::new(Some(0), Some(OTA_PROVIDER_CLUSTER), None))
            .unwrap();
